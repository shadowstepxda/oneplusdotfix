...
if device_check "cheeseburger" || device_check "OnePlus5"; then
  ...
  if [ $API -eq 25 ]; then
    ...
    ui_print "- Copying appropriate libraries"
    ui_print " "
    cp -f $TMPDIR/libs/cheeseburger/api28-29/camera.msm8998.so $TMPDIR/system/vendor/lib/hw/camera.msm8998.so
    cp -f $TMPDIR/libs/unified/api28-29/libmmcamera_imx371.so $TMPDIR/system/vendor/lib/libmmcamera_imx371.so
    cp -f $TMPDIR/libs/cheeseburger/api28-29/libmmcamera_imx398.so $TMPDIR/system/vendor/lib/libmmcamera_imx398.so
  else
    ui_print " "
  ...
  if [ $API -eq 25 ]; then
    ...
    ui_print "- Copying appropriate libraries"
    ui_print " "
    cp -f $TMPDIR/libs/dumpling/api28-29/camera.msm8998.so $TMPDIR/system/vendor/lib/hw/camera.msm8998.so
    cp -f $TMPDIR/libs/unified/api28-29/libmmcamera_imx371.so $TMPDIR/system/vendor/lib/libmmcamera_imx371.so
    cp -f $TMPDIR/libs/dumpling/api28-29/libmmcamera_imx398.so $TMPDIR/system/vendor/lib/libmmcamera_imx398.so
  else
    ui_print " "
    ui_print "- Android detected: Q (10.0)"
    ui_print " "
